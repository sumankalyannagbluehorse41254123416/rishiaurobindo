...
export async function fetchDocumentCollection(
  { host }: FetchPageParams,
  identifier: string,
) {
  const baseurl =
    process.env.CMS_BASE_URL || process.env.NEXT_PUBLIC_CMS_BASE_URL;
  ...
  try {
    const response = await axios.get(
      `${baseurl}/documents/collections/${identifier}`,
      {
        headers,
      },
    );
    ...
    console.error("Document collection fetch error:", error);
    throw error;
  }
}